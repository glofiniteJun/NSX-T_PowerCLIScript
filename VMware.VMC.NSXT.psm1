            ...
            foreach ($vm in $vms) {
                ...
                if($vm.tags -ne $null){
                    for($i=0; $i -le ($vm.tags.count); $i++){
                            $tmp.tag += "$(($vm.tags[$i]).tag) "
                    }
                }
                $vmResult+=$tmp
            }
            $vmResult | Sort-Object -Property Id |Export-Csv vm.csv -NoTypeInformation
        }
    }
}
...
Function Get-NSXTTag {
    ...
    If ($PSBoundParameters.ContainsKey("Id")) {
        If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
            $method2 = "GET"
            $memberUrl2 = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/tags/effective-resources?tag=$Id"
         

            if($Troubleshoot) {
                Write-Host -ForegroundColor cyan "`n[DEBUG] - $method2`n$memberUrl2`n"
            }

            try {
                if($PSVersionTable.PSEdition -eq "Core") {
                    $requests2 = Invoke-WebRequest -Uri $memberUrl2 -Method $method2 -Headers $global:nsxtProxyConnection.headers -SkipCertificateCheck
                } else {
                    $requests2 = Invoke-WebRequest -Uri $memberUrl2 -Method $method2 -Headers $global:nsxtProxyConnection.headers
                }
            } catch {
                if($_.Exception.Response.StatusCode -eq "Unauthorized") {
                    Write-Host -ForegroundColor Red "`nThe NSX-T Proxy session is no longer valid, please re-run the Connect-NSXTProxy cmdlet to retrieve a new token`n"
            ...
            if($requests2.StatusCode -eq 200) {
                $tagname = $Id
                $results += ($requests2.Content | ConvertFrom-Json).results | Select-Object *,@{Name='Tag';Expression={$tagname}} 
                Write-Host "TAG: $Id is Done" 
            }
            $results | Select-Object -Property Tag, target_id, target_display_name, target_type | Export-Csv "$Filename.csv" -NoTypeInformation
        }
    }
    else{
        If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
            $method = "GET"
            $memberUrl = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/tags"
            ...
            if($requests.StatusCode -eq 200) {
                (($requests.Content | ConvertFrom-Json).results).tag | ForEach-Object {
                        If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
                            $method2 = "GET"
                            $memberUrl2 = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/tags/effective-resources?tag=$($_)"
    ...
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        $method = "GET"
        $memberUrl = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/realized-state/virtual-machines?enforcement_point_path=/infra/sites/default/enforcement-points/vmc-enforcementpoint"
        ...
        if($requests.StatusCode -eq 200) {
            ($requests.Content | ConvertFrom-Json).results | Select-Object -Property external_id, tags | Export-Csv vm.csv -NoTypeInformation
            }
            
        }
    }

    Function Get-NSXTVifPerHost2 {
    Param(
        [Switch]$Troubleshoot
    )
    ...
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        ...
        if($requests.StatusCode -eq 200) {
            $vif = ($requests.Content | ConvertFrom-Json).results
            ...
            foreach ($vi in $vif) {
                ...
                if($vi.ip_address_info -ne $null){
                    for($i=0; $i -le ($vi.ip_address_info.count); $i++){
                            $tmp.Ip += "$(($vi.ip_address_info[$i]).ip_addresses) "
                    }
                }
                $vmResult+=$tmp
            }
            $vmResult | Sort-Object -Property Name | Export-Csv vif.csv -NoTypeInformation
        }
    }
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        $method = "GET"
    ...
    $result = Import-Csv vm.csv 
    $RealResult = @()
    foreach ($ins in $result) {
        $tmp = [pscustomobject][ordered] @{
            "Name" = $ins.Name;
            "Id" = $ins.Id;
            "tag" = $ins.tag;
        ...
        Import-Csv vif.csv | ForEach-Object {
            if($ins.Id -eq $($_.Name)){
                $tmp.Ip += "$(($_.Ip)) "
            }
        }
    ...
    $RealResult | Sort-Object -Property Id | Select-Object -Property Name, Ip, tag | Export-Csv final.csv -NoTypeInformation
}


Function Get-NSXTVmTagIp {
    Param(
        [Switch]$Troubleshoot
    )
    ...
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        ...
        if($requests.StatusCode -eq 200) {
            ...
            $max = 0
            foreach ($vi in $vif) {
                if($max -le $vi.ip_address_info.ip_addresses.count)
                { 
                    $max = $vi.ip_address_info.ip_addresses.count
                } 
            }
            $vmResult = @()
            foreach ($vi in $vif) {
                ...
                if($vi.ip_address_info.ip_addresses.count -ne 1){
                    for($i=0; $i -le ($max-1); $i++){
                        if(($vi.ip_address_info).ip_addresses.count -gt 0 -and $i -lt ($vi.ip_address_info).ip_addresses.count){
                            $val = ($vi.ip_address_info).ip_addresses[$i]
                        }
                        else{
                            $val = $null
                        }
                        $tmp = $tmp | Select-Object *,@{Name="ip$i";Expression={$val}}
                    }
                }
                else{
                    for($i=0; $i -le ($max-1); $i++){
                        if($i -gt 0){
                            ...
                            $val = ($vi.ip_address_info).ip_addresses
                        }
                        $tmp = $tmp | Select-Object *,@{Name="ip$i";Expression={$val}}
                    }
                }
    ...
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        ...
        if($requests.StatusCode -eq 200) {
            ...
            foreach ($vm in $vms) {
                ...
                for($i=0; $i -le ($max2-1); $i++){
                        if($vm.tags.count -gt 0 -and $i -lt $vm.tags.count){
                            $val = ($vm.tags[$i]).tag
                        }else{
                            $val = $null
                        }
                        $tmp = $tmp | Select-Object *,@{Name="tag$i";Expression={$val}}
                }   
            $vmResult+=$tmp
            }
        }
    ...
    foreach ($ins in $result) {
        ...
        for($i=0; $i -le ($max2-1); $i++){
                $data = $ins."tag$i" -split "-", 2
                $tmp = $tmp | Select-Object *,@{Name="tag$i";Expression={$data[1]}}
        }
        for($i=0; $i -le ($max-1)*5; $i++){
                $tmp = $tmp | Select-Object *,@{Name="ip$i";Expression={$null}}
        }
        Import-Csv vif.csv | ForEach-Object {
            if($ins.Id -eq $($_.Name)){
                for($i=0; $i -le ($max-1)*5; $i++){
    ...
    $RealResult | Sort-Object -Property Id | Export-Csv final.csv -NoTypeInformation
}




Function Get-NSXTDFWDetails {
<#
    .NOTES
    ===========================================================================
    Created by:    William Lam
...
#>
    param(
        [Parameter(Mandatory=$false)][String]$SectionName,
        [Switch]$Troubleshoot
    )
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
        $method = "GET"
        ...
        if($requests.StatusCode -eq 200) {
            ...
            $sections | Sort-Object -Propert display_name | select display_name, id, category | Export-Csv DFW_section.csv -NoTypeInformation
        }
    }
    $results = @()
    $maxSG1 = 0
    $maxDG1 = 0
    $maxserviceG1 = 0
    $sections | select Id | ForEach-Object{
        If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection found, please use Connect-NSXTProxy" } Else {
            $_.Id
            $method = "GET"
            $distFirewallURL = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/domains/cgw/communication-maps/$($_.Id)"
            ...
            if($requests.StatusCode -eq 200) {
                ...
                foreach ($rule in $rules | Sort-Object -Property sequence_number) {
                    $sourceGroups = $rule.source_groups
                    if($maxSG -le $sourceGroups.count)
                    { 
                        $maxSG = $sourceGroups.count
                    } 

                    $destinationGroups = $rule.destination_groups
                    if($maxDG -le $destinationGroups.count)
                    { 
                        $maxDG = $destinationGroups.count
                    } 
                    $serviceGroups = $rule.services
                    if($maxserviceG -le $serviceGroups.count)
                    { 
                        $maxserviceG = $serviceGroups.count
                    } 
                }
                if($maxSG1 -le $maxSG)
                { 
                ...
                if($maxserviceG1 -le $maxserviceG)
                { 
                    $maxserviceG1 = $maxserviceG
                } 
            }
        }
    }
                ...
                foreach ($rule in $rules | Sort-Object -Property sequence_number) {
                    ...
                    foreach ($sourceGroup in $sourceGroups) {
                        if($sourceGroup -eq "ANY") {
                            ...
                            $sourceGroupName = $sourceGroup -split "/"
                            $source += $sourceGroupName[5]
                        }
                    }
                    ...
                    foreach ($destinationGroup in $destinationGroups) {
                        if($destinationGroup -eq "ANY") {
                            ...
                            $DestGroupName = $destinationGroup -split "/"
                            $destination += $DestGroupName[5]
                        }
                    }
                    ...
                    foreach ($serviceGroup in $serviceGroups) {
                        if($serviceGroup -eq "ANY") {
                            ...
                            $serviceGroupsName = $serviceGroup -split "/"
                            $service += $serviceGroupsName[3]
                        }
                    }

                    $tmp = [pscustomobject] @{
                        DFW_Section = $_.Id;
                        SequenceNumber = $rule.unique_id;
                        Name = $rule.display_name;
                        ID = $rule.id;
                        Source = ($source | Out-String).Trim();
                    }
                    for($i=0; $i -le ($maxSG1-1); $i++){
                        $tmp = $tmp | Select-Object *,@{Name="Source$i";Expression={$source[$i]}}
                    }
                    $tmp = $tmp | Select-Object *,@{Name="Destination";Expression={($destination | Out-String).Trim()}}
                    for($i=0; $i -le ($maxDG1-1); $i++){
                        $tmp = $tmp | Select-Object *,@{Name="Destination$i";Expression={$destination[$i]}}
                    }
                    $tmp = $tmp | Select-Object *,@{Name="Services";Expression={($service | Out-String).Trim()}}
                    for($i=0; $i -le ($maxserviceG1-1); $i++){
                        $tmp = $tmp | Select-Object *,@{Name="Service$i";Expression={$service[$i]}}
                    }
                    $tmp = $tmp | Select-Object *,@{Name="Action";Expression={$rule.action}}               
                    $results+=$tmp
                }
            }
        }
    }
    $results | Export-Csv DFW_rules.csv -NoTypeInformation
}

Function Get-NSXTGroupAndService {
<#
    .NOTES
    ===========================================================================
    Created by:    William Lam
...
#>
    param(
        [Parameter(Mandatory=$false)][String]$Name,
        [Parameter(Mandatory=$false)][ValidateSet("MGW","CGW")][String]$GatewayType,
        [Switch]$Troubleshoot
    )

    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection  found, please use Connect-NSXTProxy" } Else {
        $method = "GET"
        $edgeFirewallGroupsURL = $global:nsxtProxyConnection.Server + "/policy/api/v1/infra/domains/cgw/groups"
        ...
        if($requests.StatusCode -eq 200) {
            ...
            foreach ($group in $groups) {
                ...
                foreach ($member in $group.expression) {
                    if($member.ip_addresses) {
                        $IP += $member.ip_addresses
                    } else {
                        if($member.resource_type -eq "Condition") {
                            $TAG += $member.value
                        }
                        if($member.resource_type -eq "PathExpression") {
                            $memberpath = $member.paths -split "/"
                            $SEGMENT += $memberpath[5]
                        }
                ...
                $tmp = [pscustomobject] @{
                    ...
                    SEGMENT = ($SEGMENT | Out-String).Trim();
                }
                $results+=$tmp
            }
            $results | Export-Csv Group_Members.csv -NoTypeInformation
        }
    }
    If (-Not $global:nsxtProxyConnection) { Write-error "No NSX-T Proxy Connection  found, please use Connect-NSXTProxy" } Else {
        $method = "GET"
        ...
        if($requests.StatusCode -eq 200) {
            ...
            foreach ($service in $services | Sort-Object -Propert display_name) {
                $serviceEntry = $service.service_entries
                    $serviceProtocol = $()
                    $serviceSourcePorts = $()
                    $serviceDestinationPorts =$()
                    foreach($serv in $serviceEntry){
                        $val = $serv.PSObject.Properties | Select -First 1
                        if($serv.destination_ports.count -ge 2){
                            foreach($i in $serv.destination_ports){
                                $serviceProtocol += @($val.Value)
                            }
                        }else{
                            $serviceProtocol += @($val.Value)
                        }
                        $serviceSourcePorts += @($serv.source_ports)
                        $serviceDestinationPorts += @($serv.destination_ports)
                    }
                    if($service.id -eq "MS_Exchange_2010_Unified_Messaging_Server"){
                ...
                $tmp = [pscustomobject] @{
                    Id = $service.id;
                    Protocol = ($serviceProtocol | Out-String).Trim();
                    Source = ($serviceSourcePorts | Out-String).Trim();
                    Destination = ($serviceDestinationPorts | Out-String).Trim();
                }
                $results += $tmp
            }
            $results | Export-Csv Service_Memebers.csv -NoTypeInformation
        }
    }
}
